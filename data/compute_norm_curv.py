...
def getFilenames(path, shapeName, partition='test'):
...
def readFile(path, shapeName, fileName, partition='test'):
    path = os.path.join(path, shapeName, partition, fileName)
    pc = np.load(path,allow_pickle=True)
    return pc


def saveFile(data, path, shapeName, fileName, partition='test'):
    path = os.path.join(path, shapeName, partition, fileName)
    np.save(path, data)
...
if __name__ == '__main__':
    # root = r"/root/gast/data/PointDA_data/shapenet"
    # save_root = r"/root/gast/data/PointDA_data/shapenet_norm_curv/"

    root = r"/root/gast/data/PointDA_data/modelnet/"
    save_root = r"/root/gast/data/PointDA_data/modelnet_norm_curv/"
    ...
    for j in range(len(Shape)):
        s = Shape[j]
        f = File[j]
        curv = Curv[j].reshape(2048, 1)  # note that the point number of pointcloud in shapenet is 1024
        norm = Norm[j]
        ...
        if not os.path.exists(os.path.join(save_root, s, 'test')):
            os.makedirs(os.path.join(save_root, s, 'test'))
        saveFile(data, save_root, s, f, 'test')
