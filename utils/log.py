...
class IOStream():
    ...
    def save_modele(self, model):
        path = self.path + '/model.ptex' + self.args.model
        best_model = copy.deepcopy(model)
        # torch.save(model.state_dict(), path)
        if len(self.args.gpus) > 1:
            torch.save(model.module.state_dict(), path)
    ...
    def save_model_mul(self, model):
        path = self.path + '/model.ptmul' + self.args.model
        best_model = copy.deepcopy(model)
        # torch.save(model.state_dict(), path)
        if len(self.args.gpus) > 1:
            torch.save(model.module.state_dict(), path)
    ...
    def save_model_e(self, model, i):
        path = self.path + '/model.pt' + self.args.model + str(i)
        best_model = copy.deepcopy(model)
        # torch.save(model.state_dict(), path)
        if len(self.args.gpus) > 1:
            torch.save(model.module.state_dict(), path)
    ...
    def save_model_ee(self, model, i):
        path = self.path + '/model.ptextral' + self.args.model + str(i)
        best_model = copy.deepcopy(model)
        # torch.save(model.state_dict(), path)
        if len(self.args.gpus) > 1:
            torch.save(model.module.state_dict(), path)
