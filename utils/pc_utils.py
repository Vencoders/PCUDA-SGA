...
def fps(data, number):
    ...
    fps_data = pointnet2_utils.gather_operation(data.transpose(1, 2).contiguous(), fps_idx).transpose(1,2).contiguous() ## B Gn 6
    return fps_idx, fps_data
...
def group(X, norm):
    ...
    xyz_no_normal = X.clone().contiguous()
    xyz_only_normal = norm.clone().contiguous()

    # fps the centers out
    _, center = fps(xyz_no_normal, num_group)  # B G 3
    # print("center:",center.shape)
    # knn to get the neighborhood
    knn = KNN(group_size, transpose_mode=True)
    _, idx = knn(xyz_no_normal, center)  # B G M
    # print("idx",idx.shape)
    ...
    neighborhood_no_normal = neighborhood_no_normal.view(batch_size, num_group, group_size, 3).contiguous()
    # normalize
    ...
    neighborhood_only_normal = neighborhood_only_normal.view(batch_size, num_group, group_size,
                                                             3).contiguous()
    # neighborhood_no_normal[:, :, :, :3] = neighborhood[:, :, :, :3].contiguous() - center.unsqueeze(2)[:, :, :, :3].contiguous()
    # print("neighborhood_no_normal", neighborhood_no_normal.shape)
    # print("neighborhood_only_normal", neighborhood_only_normal.shape)
    print("center", center.shape)
...
def groupx(X):
    ...
    # print("xxxxx:",X.shape)
    batch_size, num_points, _ = X.shape
    xyz_no_normal = X[:, :, :3].clone().contiguous()
    xyz_only_normal = X[:, :, 3:6].clone().contiguous()
    ...
    neighborhood_only_normal = xyz_only_normal.view(batch_size * num_points, -1)[idx, :]
    neighborhood_only_normal = neighborhood_only_normal.view(batch_size, num_group, group_size,3).contiguous()
    # neighborhood_no_normal[:, :, :, :3] = neighborhood[:, :, :, :3].contiguous() - center.unsqueeze(2)[:, :, :, :3].contiguous()
    # print("neighborhood_no_normal", neighborhood_no_normal.shape)
    # print("neighborhood_only_normal", neighborhood_only_normal.shape)
    # print("center", center.shape)
...
def farthest_point_sample_np(xyz, npoint):
    ...
    for i in range(npoint):
        ...
        # print("mask----------:",mask)
        distance[mask] = dist[mask]  # save the minimal distance of each point from all points that were chosen until now
        # print("disma-----:",distance[mask])
        # print("dissh----:",distance[mask].shape)
        farthest = np.argmax(distance, axis=1)  # get the index of the point farthest away
        # print("ff----:",farthest)
        # print("centroids",centroid.shape)
        # print("centroids_vals",centroids_vals.shape)
    return centroids, centroids_vals
