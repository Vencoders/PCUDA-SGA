...
import numpy as np
import random
from utils.data_utils import PointcloudScale, PointcloudToTensor, PointcloudTranslate,PointcloudJitter, \
    PointcloudRotate, PointcloudRotatePerturbation, PointcloudRandomCrop, PointcloudUpSampling,PointcloudRandomCropEdge,\
    PointcloudRandomCropWeak,PointcloudFlip,PointcloudSelfOcclusion,PointcloudDensity,PointcloudDensityWeak,PointcloudDrop,PointcloudDropWeak,\
    PointcloudSaltPepperNoise, PointcloudDegradationNoise
...
trans1_2 = transforms.Compose(
    [
        PointcloudToTensor(),
        PointcloudSaltPepperNoise(),
    ])

trans1_0 = transforms.Compose(
    [
        PointcloudToTensor(),
        PointcloudDegradationNoise()
    ])



trans1_3 = transforms.Compose(
    [
        PointcloudToTensor(),
        PointcloudRandomCrop(),
        PointcloudScale(),
    ])

trans1_4 = transforms.Compose(
    [
        PointcloudToTensor(),
        PointcloudRandomCropWeak(),
    ])
...
def load_moco(base_encoder, args):
    ...
    checkpoint = torch.load('./experiments/GAST/' + args.model_file)
    print(checkpoint)
...
def mask_correlated_samples(batch_size, labels):
    ...
    mem_labels = labels.reshape(-1, 1)
    mask[:, 0:] = mem_labels != labels

    mask_m = torch.ones((batch_size*2, batch_size*2)).bool()
    mask_m = mask_m.fill_diagonal_(0)
    for i in range(batch_size):
        mask_m[i, batch_size+i] = 0
        mask_m[batch_size+i, i] = 0

    mask = mask[mask_m].reshape(batch_size * 2, -1)   
...
def view1(pointcould):
    # view
    # pcd1 = pointcould.numpy()
    pcd1 = pointcould


    fig1, ax1 = plt.subplots(subplot_kw={"projection": "3d"})
    ax1.scatter3D(pcd1[:,0],pcd1[:,1],pcd1[:,2])
    ax1.set_xlabel("X")
    ax1.set_ylabel("Y")
    ax1.set_zlabel("Z")
    ax1.set_title("PC1")
...
def MixupS1(pointcloud):


    # crop, remain = trans1_1(pointcloud)
    pc0 = pointcloud.copy()
    pc1 = pointcloud.copy()

    point_t1 = trans1_1(pc0)
    point_t2 = trans1_1(pc1)
    # point_t3 = trans1_3(pointcloud)


    point_t1 = point_t1.clone().cpu().numpy()
    point_t2 = point_t2.clone().cpu().numpy()
    # point_t3 = point_t3.clone().cpu().numpy()


    num_points, _ = pointcloud.shape

    # print(point_t1.shape)

    num_pt1, _ = point_t1.shape

    # draw lambda from beta distribution
    # lam = np.random.beta(1, 1)
    # lam = random.uniform(0.3, 0.7)

    num_pts_a = round(0.7 * num_pt1)
    num_pts_b = num_points - num_pts_a


    point_t1 = np.swapaxes(np.expand_dims(point_t1, 0), 1, 2)
    point_t2 = np.swapaxes(np.expand_dims(point_t2, 0), 1, 2)
    # point_t3 = np.swapaxes(np.expand_dims(point_t3, 0), 1, 2)


    # print("t1",point_t1.shape)
    # first sample
    _, point_t11 = farthest_point_sample(point_t1, num_pts_a)
    _, point_t12 = farthest_point_sample(point_t2, num_pts_b)
    # _, point_t13 = farthest_point_sample(point_t3, num_pts_a)


    point_t11 = np.transpose(point_t11, (0, 2, 1))  # [B, N1, C]
    point_t12 = np.transpose(point_t12, (0, 2, 1))  # [B, N2, C]
    # point_t13 = np.transpose(point_t13, (0, 2, 1))  # [B, N3, C]


    point_new1 = np.concatenate((point_t11, point_t12), 1).astype('float32')  # convex combination [B, N, C]
    # print("---",point_new.shape)
    point_new1 = np.transpose(point_new1, (0, 2, 1))
    point_new1 = np.swapaxes(point_new1.squeeze(), 1, 0).astype('float32')

    # point_new2 = np.concatenate((point_t13, point_t12), 1).astype('float32')  # convex combination [B, N, C]
    # # print("---",point_new.shape)
    # point_new2 = np.transpose(point_new2, (0, 2, 1))
    # point_new2 = np.swapaxes(point_new2.squeeze(), 1, 0).astype('float32')

    pcn1 = point_new1.copy()
    pcn2 = point_new1.copy()

    # if sr == 1:
    point_m1 = trans1_3(pcn1)
    point_m2 = trans1_4(pcn2)
    # else:
    #     point_m1 = trans1_5(pcn1)
    #     point_m2 = trans1_6(pcn2)

    return point_m1, point_m2



def MixupA2(pointcloud):
    ...
    point_t1 = trans1_1(pc0)
    point_t2 = trans1_2(pc1)
    point_t3 = trans1_0(pc3)
    ...
    num_pts_a = round(0.8 * num_pt1)
    num_pts_b = round(0.1 * num_pt2)
    num_pts_c = num_points - num_pts_a - num_pts_b

    # print(num_pts_a,num_pts_b,num_pts_c)
    ...
    _, point_t11 = farthest_point_sample(point_t1, num_pts_a)
    _, point_t12 = farthest_point_sample(point_t2, num_pts_b)
    _, point_t13 = farthest_point_sample(point_t3, num_pts_c)
    ...
    point_new1 = np.concatenate((point_t11, point_t12, point_t13), 1).astype('float32')  # convex combination [B, N, C]
    # print("---",point_new1.shape)
    point_new1 = np.transpose(point_new1, (0, 2, 1))
    point_new1 = np.swapaxes(point_new1.squeeze(), 1, 0).astype('float32')
